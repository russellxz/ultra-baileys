...
import { buildProfilePictureQueryContent, buildTcTokenFromJid } from '../Utils/tc-token-utils'
import {
	type BinaryNode,
	getBinaryNodeChild,
...
export const makeChatsSocket = (config: SocketConfig) => {
	...
	const profilePictureUrl = async (jid: string, type: 'preview' | 'image' = 'preview', timeoutMs?: number) => {
		...
		let content: BinaryNode[] | undefined = [{ tag: 'picture', attrs: { type, query: 'url' } }]

		if (serverProps.profilePicPrivacyToken && isUserJid && !isSelf) {
			const pictureContent = await buildProfilePictureQueryContent(authState, normalizedJid, type, getLIDForPN)
			content = pictureContent ?? content
		}
